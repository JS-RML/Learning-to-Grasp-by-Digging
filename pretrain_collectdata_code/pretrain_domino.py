...
example_number_need_collect = 150
    ...
    #for i in range(2,num_obj+2):
        ...
        if np.sum(mask) >= mask_accept_thed:
            ...
            if obj_shape == 'domino':
                ...
                if np.dot(obj_y_in_w[:3],np.array([0,0,1])) > 0:
                    obj_y_in_w = -obj_y_in_w
                abs_yaw = math.acos(np.dot(obj_y_in_w[:3]/linalg.norm(obj_y_in_w[:3]),np.array([0,1,0])))
                if np.dot(obj_y_in_w[:3],np.array([1,0,0])) > 0:
                    yaw_short = abs_yaw
                else:
                ...
                d = 4
                [c_y,c_x] = find_center(mask)
                good_pt_x = int(c_x+(d+1)*cos(yaw_short-math.pi/2))
                good_pt_y = int(c_y+(d)*sin(yaw_short-math.pi/2))
                bad_pt_x = int(c_x-d*cos(yaw_short-math.pi/2))
                bad_pt_y = int(c_y-d*sin(yaw_short-math.pi/2))
                # Mesh grid for bad points
                bad_area_h = 2#35
                bad_area_w = 2#25
                good_area_h = 2
                ...
                if debug_display == 1:
                    ...
                    cv2.putText(depth_copy,str(math.degrees(yaw_short)), (100,100), cv2.FONT_HERSHEY_SIMPLEX, 1, 255)
                    ...
                    depth_save.save('./pre_data'+str(loop_id)+'/train/'+'floder_'+str(floder_id)+'_obj_'+str(i)+'_yawLong_'+str(rot_ind_long)+'_yawShort_'+str(rot_ind_short)+'.png','PNG')
    ...
    for i in range(num_rotation):
        for pt in p_ws:
            for rt in r_ws:
                for ap_ind in ap_ws:
                    for fl_ind in fl_ws:
                        if (ap_ind!=0) and (fl_ind==2 or fl_ind==3) and np.min(label_long[:,:,i])<255:
                            img_d_rot_temp = Image.fromarray(img_d)
                            img_d_rot = img_d_rot_temp.rotate(i*60, fillcolor=(255,255,255))
                            img_d_rot_path = img_save_dir+'domino_'+'num_'+str(floder_id)+'_yaw_'+str(int(i*60)) \
                            ...
                            label_long_path =label_save_dir+'domino_'+'num_'+str(floder_id)+'_yaw_'+str(int(i*60)) \
                                                        +'_ap_'+str(int(ap_ind))+'_pitch_'+str(int(pt)) \
                                                        +'_roll_'+str(int(rt))+'_fl_'+str(int(fl_ind))+ '_long.png'
                            ...
                            grasp_paras_save_path = sec_input_dir+'domino_'+'num_'+str(floder_id)+'_yaw_'+str(int(i*60)) \
                                                        +'_ap_'+str(int(ap_ind))+'_pitch_'+str(int(pt)) \
                                                        +'_roll_'+str(int(rt))+'_fl_'+str(int(fl_ind))+'_long.npy'
                            np.save(grasp_paras_save_path, grasp_paras.astype(np.int))

                        if (ap_ind==0 or ap_ind==1) and (fl_ind!=3) and np.min(label_short[:,:,i])<255:
                            img_d_rot_temp = Image.fromarray(img_d)
                            img_d_rot = img_d_rot_temp.rotate(i*60, fillcolor=(255,255,255))
                            img_d_rot_path = img_save_dir+'domino_'+'num_'+str(floder_id)+'_yaw_'+str(int(i*60)) \
                                                        +'_ap_'+str(int(ap_ind))+'_pitch_'+str(int(pt)) \
                                                        +'_roll_'+str(int(rt))+'_fl_'+str(int(fl_ind))+'_short.png'
                            img_d_rot.save(img_d_rot_path,'PNG')
                            label_short_temp = Image.fromarray(label_short[:,:,i])
                            label_rot_short = np.array(label_short_temp.rotate(angle=i*60, fillcolor=255))
                            label_short_path =label_save_dir+'domino_'+'num_'+str(floder_id)+'_yaw_'+str(int(i*60)) \
                                                        +'_ap_'+str(int(ap_ind))+'_pitch_'+str(int(pt)) \
                                                        +'_roll_'+str(int(rt))+'_fl_'+str(int(fl_ind))+'_short.png'
                            ...
                            grasp_paras_save_path = sec_input_dir+'domino_'+'num_'+str(floder_id)+'_yaw_'+str(int(i*60)) \
                                                        +'_ap_'+str(int(ap_ind))+'_pitch_'+str(int(pt)) \
                                                        +'_roll_'+str(int(rt))+'_fl_'+str(int(fl_ind))+'_short.npy'
                            np.save(grasp_paras_save_path, grasp_paras.astype(np.int))
    p.disconnect()
    gc.collect()
